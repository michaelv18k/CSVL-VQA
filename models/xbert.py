...
# Modify this to automatically save attention maps
class BertSelfAttention(nn.Module):
    def __init__(self, config, is_cross_attention):
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(config, "embedding_size"):
        ...
        self.dropout = nn.Dropout(config.attention_probs_dropout_prob)
        self.save_attention = True  # <-- Set True by default
    ...
    def forward(
        ...
        # Save attention maps to disk if enabled
        if self.save_attention:
            if not os.path.exists("attention_maps"):
                os.makedirs("attention_maps")
            torch.save(attention_probs.cpu(), f"attention_maps/attention_map.pt")  # Save to disk
            self.attention_map = attention_probs.detach().cpu()  # Also keep in memory
